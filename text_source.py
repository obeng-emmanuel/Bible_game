...
class LocalJsonSource:
    """
    Loads Bible & SOP from local JSON files.

    Bible JSON shape per book:
      { "book": "Mark", "chapters": [["v1","v2",...], ["v1","v2",...], ...] }

    SOP JSON shape per title:
      { "title":"Steps to Christ", "chapters":[{"title":"...","text":"..."}, ...] }
    """
    ...
    def passage_text(self, reference: str) -> str:
        """
        Accepts "Mark 5" or "John 3:16-18" and returns joined text.
        """
        m = _BOOK_REF_RE.match(reference or "")
        if not m:
            raise ValueError(f"Unrecognized reference: {reference!r}")
    ...
    # ---------- sop helpers ----------
    def sop_books(self) -> List[str]:
        return sorted(self._egw_index.keys())

    def sop_chapter_text(self, title: str, chapter_index: int) -> str:
        if title not in self._egw_index:
            raise ValueError(f"SOP book not found: {title}")
        chapters = self._egw_index[title]
        if chapter_index < 1 or chapter_index > len(chapters):
            raise ValueError(f"{title} has only {len(chapters)} chapters.")
        return chapters[chapter_index - 1].get("text", "") or ""
